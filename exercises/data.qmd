...
(@)   Cleveland Heart Disease (ISLR) @#HeartDisease-data
(@)   Medical Expenditure Panel Survey (heckmanGE) @#MEPS-data
(@)   Mayo Clinic Primary Biliary Cholangitis Data (Survival) @#survival-data
(@)   Sleep Study (lme4) @#sleep-data
(@)   Diabetes study @#diabetes-data
(@)   Cognitive weight loss RCT @#weight-data
(@)   FEV1 COPD simulation data () @#fev1-data
...
**Outcome Variable**
-   A HD (or HD) – Heart Disease Status (Confirmed coronary artery disease based on angiography (gold standard)).
...
```{.r filename="my-report.qmd"}
hd <- readr::read_csv("https://www.statlearning.com/s/Heart.csv")
```
...
```{.r filename="my-report.qmd"}
#data_url <- "https://raw.githubusercontent.com/cran/ssmrob/master/data/MEPS2001.rda"
...
**Description**
The average reaction time per day (in milliseconds) for subjects in a sleep deprivation study. Stress reduction techniques
...
post_meal_insulin <- readr::read_csv2("https://zenodo.org/records/8292712/files/SUA_CVDs_risk_factors.csv")
```
...
The dataset is made within a collaboration by people from major pharmaceutical companies, such as Eli Lilly and Company, Boehringer Ingelheim Pharma GmbH & Co, Gilead Sciences, Inc., F. Hoffmann-La Roche AG, Merck Sharp & Dohme, Inc., AstraZeneca plc, and inferential.biostatistics GmbH.
