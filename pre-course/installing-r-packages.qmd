...
them, you need to install the R packages we'll use in the course. R
...
-   **R code** is any individual piece of text that R interprets as
    ...
    -   R code is split into two types: objects (like a dataset) or
        ...
        code with `()` at the end (at least in this course), that
...
For this course, we will be focusing on R packages that are powerful
...
`{medisr}` helper package that we use for this course to help you install all
the needed packages and to run some basic checks during the pre-course
...
Console (panel "B" in @fig-rstudio-introduction) to install the `{medisr}`
package. Hit "Enter" and the `{medisr}` helper package will be installed.
...
#| filename: "Console"
medisr::install_packages()
```

::: callout-important
This step sometimes fails for some people because their computer
...
#| filename: "Console"
install.packages(c("tidyverse", "here", "survival", "prodigenr", "styler"))
```
:::
