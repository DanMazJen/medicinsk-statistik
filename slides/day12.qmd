...
model_rr <- glm(seizure_count ~ drug + baseline_seizures,
                     data = epil,
                     family = poisson(link = "log"))

# 2. Compute Average Risk Ratio (Relative Risk) using robust standard errors
avg_comparisons(model_rr,
                variables = "drug",
                comparison = "lnratioavg", # Calculates log risk ratio, then exponentiates
                vcov = sandwich::vcovHC(model_rr, type = "HC3")) # Applies robust standard errors
```
